...
if [ ! -d "$CONFDIR" ]; then
    ...
    mkdir -p "$CONFDIR"
    timeout 60 /etc/init.d/dnsmasq restart
    exit 0
fi
...
if ! pgrep -f chinadns-ng >/dev/null 2>&1; then
    ...
    timeout 120 /etc/init.d/passwall restart
    exit 0
fi

# ── ответ резолвера ──────────────────────────────────────────────
# timeout на рестартах: при сломанной конфигурации init-скрипт умеет
# виснуть на procd-локе и держать его до перезагрузки.
