...
# Живое зеркало списков. Тем же способом доставки и по той же причине:
# своей строки в cron у проданных роутеров не будет никогда. Внутри
# стоит пауза в час, так что в обычную минуту это одно [ -f ].
[ -x /usr/bin/titan_lists_mirror.sh ] && /usr/bin/titan_lists_mirror.sh
